...
class Extractor():
    def __init__(self, frame_len_sec):
        self.frame_len_sec = frame_len_sec

    def extract_features(self, wav_path):
        y, sr = librosa.load(wav_path, sr=None)
        frame_len = int(sr * self.frame_len_sec)
        mfcc = [librosa.feature.mfcc(y[frame:frame + frame_len], sr).mean(axis=1) for frame in range(0, len(y) - frame_len + 1, frame_len)]
        mfcc = np.vstack(mfcc)
        
        melspec = [librosa.feature.melspectrogram(y[frame:frame + frame_len], sr).mean(axis=1) for frame in range(0, len(y) - frame_len + 1, frame_len)]
        melspec = np.vstack(melspec)
        melspec = np.log10(melspec)
        self.n, self.m = mfcc.shape[1], melspec.shape[1]
        
        return pd.DataFrame(np.hstack([mfcc, melspec]))
...
class SSPNetDataSampler:
    ...
    def df_from_file(self, wav_path, frame_sec, extractor):
        """
        ...
        """
        data = extractor.extract_features(wav_path)
    ...
    def create_sampled_df(self, frame_sec, naudio=None, save_path=None, force_save=False):
        ...
        extractor = Extractor(frame_sec)
        dataframes = [self.df_from_file(wav_path, frame_sec, extractor) for wav_path in fullpaths]
        df = pd.concat(dataframes)

        cols = [f'MFCC_{i}' for i in range(extractor.n)] + [f'SPEC_{i}' for i in range(extractor.m)]
        cols.append("IS_LAUGHTER")
        cols.append("SNAME")
        df.columns = cols
